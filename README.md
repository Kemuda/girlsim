...
- **可玩的 MVP 已上线**：完整的游戏流程已经跑通——7 个人生阶段、14 个场景、命运卡、结局判定都可以体验。欢迎先玩一遍，感受一下这个游戏长什么样。
- **正在做的事：设计剧情和内容**：MVP 里的文案是占位性质的，现在的重点工作是设计角色宇宙（17 个女性角色的六维面板和人生故事线）、打磨场景文案和选项、以及完善游戏引擎机制（影子代价系统、叙事感知系统等）。详见 `docs/` 目录。

简单说：**骨架搭好了，现在在填血肉。** 这个阶段特别适合一起参与——不管是写故事、设计角色、还是讨论游戏机制，都是最好的时机。
