...
export function judgeStrength(chart: BaZiChart): StrengthResult {
  ...
  for (const { pos, p } of pillars) {
    ...
    if (stemRel === 'generate') {
      shengScore += 30 * w;
      shengParts.push(`${pos === 'year' ? '年' : pos === 'month' ? '月' : '时'}干${p.stem.name}生${dm.wuxing}`);
    }
    // Check branch main qi
  ...
  for (const { pos, p } of pillars) {
    const w = POSITION_WEIGHT[pos];
    if (p.stem.wuxing === dm.wuxing) {
      zhuScore += 35 * w;
      zhuParts.push(`${pos === 'year' ? '年' : pos === 'month' ? '月' : '时'}干${p.stem.name}同类`);
    }
    for (const cg of p.branch.canggan) {
      const cgStem = getTianGan(cg.name);
